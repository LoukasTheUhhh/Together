...
function highlightTogether(code) {
  // Escape HTML
  code = code.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");

  // Comments (//, $$, or block ><< >>)
  code = code.replace(/(\$\$.*$|\/\/.*$)/gm, '<span class="tgt-comment">$1</span>');
  code = code.replace(/(&gt;&lt;[\s\S]*?&gt;&lt;)/gm, '<span class="tgt-comment">$1</span>');

  // Implementations
  code = code.replace(/(^|\s)(!implement\s+condition\s+(normal|looping))/gm, '$1<span class="tgt-implement">$2</span>');

  // ++action start/end
  code = code.replace(/(\+\+action\s+start)/g, '<span class="tgt-actionstart">$1</span>');
  code = code.replace(/(\+\+action\s+end|\-\-action\s+end)/g, '<span class="tgt-actionend">$1</span>');

  // Conditions (If, Else If, Else, During, For)
  code = code.replace(/\b(If|Else\s+If|Else|During|For)\b/g, '<span class="tgt-condition">$1</span>');

  // Keywords (Action, Storage, Runner, Process, Connect, Let, log)
  code = code.replace(/\b(Action|Storage|Runner|Process|Connect|Let|log)\b/g, '<span class="tgt-keyword">$1</span>');

  // Variables ([var])
  code = code.replace(/(\[[a-zA-Z0-9_]+\])/g, '<span class="tgt-variable">$1</span>');

  // Strings (single or double quotes, or *...*)
  code = code.replace(/("[^"]*"|'[^']*'|\*[^\*]*\*)/g, '<span class="tgt-string">$1</span>');

  // Numbers (not inside quotes or stars)
  code = code.replace(/(?<=\s|\[|=)(\d+(\.\d+)?)(?=\s|,|\]|\}|$)/g, '<span class="tgt-number">$1</span>');

  // Default styling for any remaining text
  // Not strictly necessary, but you may wrap all lines in .tgt-default if you wish

  return code;
}

// Live preview: highlight as user types
document.getElementById("editor").addEventListener("input", function() {
  document.getElementById("highlighted").innerHTML = highlightTogether(this.value);
});

// Initial highlight if the editor isn't empty
document.getElementById("highlighted").innerHTML = highlightTogether(document.getElementById("editor").value);
